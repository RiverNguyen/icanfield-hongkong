...
const SearchPassport = ({setCodePostal}: IProps) => {
  ...
  return (
    <div className='rounded-[1.25rem] bg-white p-[1.5rem] shadow-[0px_4px_24px_0px_rgba(0,0,0,0.04)]'>
      <h2 className='font-medium text-Phase-1-Brown heading3'>
        Tra cứu thông tin hộ chiếu
      </h2>
      <Form {...form}>
        <form
          onSubmit={form.handleSubmit(onSubmit)}
          className='flex items-end space-x-[0.5rem]'
        >
          <FormField
            ...
            render={({field}) => (
              <FormItem className='flex flex-col space-y-0'>
                <FormLabel className='mb-[0.75rem] mt-[1.5rem] inline-block font-optima text-tagtext body-14-m'>
                  Quốc gia hộ chiếu
                </FormLabel>
                <Popover>
                  <PopoverTrigger asChild>
                    <FormControl>
                      <Button
                        ...
                        role='combobox'
                        className={cn(
                          'h-[3rem] w-[17.6875rem] justify-between rounded-[0.5rem] border-none bg-[#F6F6F6] outline-none',
                  ...
                  <PopoverContent className='w-[17.6875rem] bg-white p-0'>
                    <Command>
          ...
          <button
            type='submit'
            className='flex h-[3rem] flex-1 items-center justify-between rounded-[0.5rem] bg-[linear-gradient(95deg,#95502F_-4.54%,#F5C178_95.42%)] px-[1.5rem]'
          >
