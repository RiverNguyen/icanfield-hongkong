...
// Phone number validation regex - extracted for performance and maintainability
// This regex validates international phone numbers with country codes
const PHONE_REGEX =
...
const FormConnect = () => {
  ...
  const formSchema = z.object({
    username: z
      .string()
      .min(2, {message: t('ho_ten_phai_co_it_nhat_2_ky_tu')}) // Tùy chỉnh message cho min
      .max(50, {message: t('ten_nguoi_dung_khong_duoc_vuot_qua_50_ky_tu')}) // Thêm ràng buộc max và message
      .nonempty({message: t('vui_long_nhap_ten_nguoi_dung')}), // Tùy chỉnh message cho trường bắt buộc (nonempty)
    email: z
      .string()
      .email({message: t('email_khong_hop_le')}) // Tùy chỉnh message cho email
      .nonempty({message: t('vui_long_nhap_email')}), // Tùy chỉnh message cho trường bắt buộc (nonempty)
    phone: z
      .string()
      .min(1, {message: t('vui_long_nhap_so_dien_thoai')})
      .regex(PHONE_REGEX, {
      ...
      <div className='relative z-20 h-max flex-1 p-[1.5rem_5rem] pl-0 sm:mt-[6.5rem] xsm:p-[1.5rem_1rem_2.5rem_1rem]'>
        ...
        <Form {...form}>
          ...
          >
            ...
            />
            <div className='mt-[1.5rem] flex w-full space-x-[1rem] xsm:mt-[0.75rem] xsm:space-x-[0.5rem]'>
              <FormField
                control={form.control}
                name='email'
                render={({field}) => (
...
function LoadingSpinner({className}: {className?: string}) {
  ...
  return (
    <div role='status'>
      ...
      <span className='sr-only'>{t('dang_gui')}</span>
    </div>
  )
}
