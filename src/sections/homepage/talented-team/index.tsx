			...
			<div className='relative mx-auto mt-[2.5rem] w-[83.75rem] section-container xsm:mt-[1.5rem] xsm:w-full xsm:px-0'>
				<Suspense fallback={<div>{t('dang_tai')}</div>}>
					<Swiper
						...
						} // Cập nhật trạng thái activeIndex
						breakpoints={{
							0: {
								slidesPerView: 1.5,
							},
						...
						{Array.isArray(talented_team) &&
							talented_team.map(
								(
									...
									>
										{item.link ? (
											<Link href={`${slugLocale}/${item.link.url}`}>
										...
										<div className='flex cursor-pointer flex-col items-center justify-center !overflow-visible'>
											...
											/>
											<div
												className={`flex flex-col items-center justify-center transition-opacity duration-300 ${activeIndex === index
													? 'xsm:opacity-100'
