    ...
    <section className='talented-team py-[6.5rem] xsm:py-[2.5rem] xsm:pb-0'>
      ...
      <div className='relative mx-auto mt-[2.5rem] w-[83.75rem] section-container xsm:mt-[1.5rem] xsm:w-full xsm:px-0'>
        <Suspense fallback={<div>Loading...</div>}>
            ...
            modules={[Navigation]}
            className='mySwiper'
            onSlideChange={(swiper: { realIndex: number }) => setActiveIndex(swiper.realIndex)} // Cập nhật trạng thái activeIndex
            breakpoints={{
              0: {
              slidesPerView: 1.5,
              },
            ...
            >
            {talented_team.map((item: { image: Media; name: string; position: string }, index: number) => (
              <SwiperSlide
              key={index}
