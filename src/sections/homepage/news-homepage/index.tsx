...
export interface INewsFlowProps {
  title: string
  itemsNews: ItemNews[]
  itemsNewsFeatured: ItemNewsFeatured[]
  taxonomies: {
    title: string
    slug: string
  }[]
...
const NewsFlow: FC<INewsFlowProps> = ({
  title,
  itemsNews,
  itemsNewsFeatured,
  taxonomies,
  ...
  return (
    <section className='space-y-[1.5rem] bg-white pt-[2.5rem] shadow-[0px_-20px_40px_0px_rgba(0,0,0,0.03)] sm:space-y-[2.5rem] sm:rounded-[4rem_4rem_0rem_0rem] sm:p-[5rem_0_6.5rem] xsm:mb-[4rem]'>
      ...
      <div className='relative flex max-w-[95rem] flex-wrap justify-between pb-[4rem] sm:ml-auto'>
        ...
        <div className='px-[1rem] sm:max-w-[21.38rem] sm:pr-[0.38rem]'>
          ...
          <p className='body-14 mb-[2.2rem] mt-[0.5rem] text-black/60'>
            Cập nhật những tin tức mới nhất từ các sự kiện và các thông tin ấn
            tượng về hoạt động của iCanfield.
          </p>
  ...
  return (
    <div className='group relative h-[15rem] flex-none overflow-hidden rounded-[1rem] sm:h-[22.125rem] sm:flex-1 sm:first:mr-[1.5rem] xsm:first:mb-[1rem]'>
      ...
      <div className='absolute left-0 top-0 flex h-full w-full flex-col p-[1rem] sm:p-[1.5rem]'>
        ...
        <Link href={link} className='mt-auto inline-flex cursor-pointer items-center self-start rounded-[0.5rem] border border-white/25 p-[0.84rem_0.75rem_0.84rem_1.5rem] xsm:hidden relative z-10 group/btn hover:bg-white'>
          <span className='body-14-m text-white group-hover/btn:text-greentext'>Chi tiết bài viết</span>
...
function LatestNews({title, image, date, link}: ItemNews) {
  return (
    <div>
      <ImageV2
        src={image.src}
