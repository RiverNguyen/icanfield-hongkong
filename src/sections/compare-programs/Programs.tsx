...
export default function Programs({
	...
	programs: DataItem[]
	dataTitleCompare: acfPage['title_compare']
}) {
	// console.log(dataTitleCompare)
	const [indexProgramsActive, setIndexProgramsActive] = useState<number[]>([
		-1, -1, -1, -1,
	])
	...
	useEffect(() => {
		if (
			...
			setKeys([]) // Đảm bảo setKeys luôn nhận một mảng hợp lệ
		}
	}, [programs])
	...
	return (
		<div
			id='table__programs'
			className='hidden-scrollbar relative h-fit w-full px-[1rem] section-container xsm:overflow-x-auto'
		>
			<div className='h-fit w-fit border border-solid'>
					...
					<div className='flex w-[calc(19.375rem*4)] flex-shrink-0 bg-background xsm:w-[calc(13.125rem*4)]'>
						{indexProgramsActive?.map((item, index: number) => (
							...
							>
								...
								>
									<SelectTrigger className='h-[3rem] w-full rounded-[0.75rem] border-none bg-white shadow-[0px_1.203px_4.812px_0px_rgba(0,0,0,0.10)] outline-none ring-0 focus:ring-0 [&_p]:[&[data-placeholder]]:hidden'>
										<SelectValue
											placeholder={`${programs?.[indexProgramsActive[index]]?.title || t('chon_chuong_trinh')}`}
				...
				<div className='relative grid h-fit w-full grid-cols-[1fr_19.375rem_19.375rem_19.375rem_19.375rem] xsm:grid-cols-[6.25rem_13.125rem_13.125rem_13.125rem_13.125rem]'>
					...
					{indexProgramsActive?.map((item: number, idx: number) => (
						...
						>
							{programs?.[item] && <CardPrograms data={programs?.[item]} />}
						</div>
					))}
					{Array.isArray(keys) &&
						(check ? keys : keys.slice(0, 10))?.map(
							(item: string, idx: number) => (
								<Fragment key={idx}>
									<div
										className={cn(
											'border-t p-[1.25rem_1rem] text-[1rem] font-semibold leading-normal tracking-[-0.02rem] text-[#121212DE] xsm:p-[0.75rem]',
											programs?.[0]?.compare?.[item] === null && 'bg-[#f8f9fa]',
										)}
									>
										{dataTitleCompare?.[item]}
									</div>
									{indexProgramsActive.map((id: number, index: number) => (
										<Fragment key={item + index}>
											{programs?.[0]?.compare?.[item] === null ? (
												<div
													className={cn(
														'border-t border-solid bg-[#f8f9fa]',
														index === 0 && 'border-l',
													)}
												></div>
											) : (
												<div
													dangerouslySetInnerHTML={{
														__html: programs?.[id]?.compare?.[item] || '',
													}}
