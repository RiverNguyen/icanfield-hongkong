...
// Mapping function to convert Chinese location names to English names for GeoJSON matching
const mapChineseToEnglishLocation = (chineseName: string): string => {
	const locationMap: { [key: string]: string } = {
		...
		'澳洲首都地区': 'Australian Capital Territory',
		'行动': 'Australian Capital Territory',
		'行動': 'Australian Capital Territory',
		// Tasmania
		'塔斯马尼亚': 'Tasmania',
		'塔斯馬尼亞': 'Tasmania',
		// Northern Territory
	...
	// Check if the Chinese name exists in the map
	if (locationMap[chineseName]) {
		return locationMap[chineseName]
	}

	// If not found, return the original name (might already be in English)
	return chineseName
}
...
const PioneeringValuesAustralia = ({
	...
	// Normalize dataMap: convert Chinese location names to English for GeoJSON matching
	// Group by slug to avoid duplicates and merge counts
	const normalizedDataMap = React.useMemo(() => {
		...
		const locationMap = new Map<string, {
			slug: string
			location_name: string
			location_name_en: string // English name for GeoJSON matching
			count: number
			metropolis?: string
		}>()

		dataMap.forEach((item) => {
			const englishName = mapChineseToEnglishLocation(item.location_name)
			const key = item.slug
			const isChineseName = englishName !== item.location_name // Check if it's a Chinese name

			if (locationMap.has(key)) {
				// If slug already exists, merge the data
				const existing = locationMap.get(key)!
				const existingIsChinese = existing.location_name_en !== existing.location_name

				// Prefer Chinese name for display, higher count, and better metropolis info
				const shouldUpdate =
					item.count > existing.count ||
					(isChineseName && !existingIsChinese) || // Prefer Chinese name
					(item.count === existing.count && isChineseName && !existingIsChinese) ||
					(!existing.metropolis && item.metropolis) // Prefer item with metropolis

				if (shouldUpdate) {
					locationMap.set(key, {
						...existing,
						count: Math.max(item.count, existing.count), // Use max count
						location_name: isChineseName ? item.location_name : existing.location_name, // Prefer Chinese for display
						location_name_en: englishName, // Always update English name
						metropolis: item.metropolis || existing.metropolis,
					})
				} else if (!existing.location_name_en || existing.location_name_en === existing.location_name) {
					// Update English name if not set or if current is better
					locationMap.set(key, {
						...existing,
						location_name_en: englishName,
						count: Math.max(item.count, existing.count),
						metropolis: item.metropolis || existing.metropolis,
					})
				}
			} else {
				locationMap.set(key, {
					slug: item.slug,
					location_name: item.location_name, // Original name for display
					location_name_en: englishName, // English name for GeoJSON matching
					count: item.count,
					metropolis: item.metropolis,
				})
			}
		})
		...
		<section className='bg-background pt-[3rem] sm:-translate-y-[3rem]'>
			...
			<div className='relative h-[41.5rem] w-full overflow-hidden rounded-[1.25rem] bg-white section-container xsm:h-[20.4rem] xsm:w-full xsm:rounded-none'>
				<LeafletMapCountries
					...
					dataCountry={normalizedDataMap.map(item => ({
						slug: item.slug,
						location_name: item.location_name, // Original name for display
						location_name_en: item.location_name_en, // English name for GeoJSON matching
						count: item.count,
						metropolis: item.metropolis,
					}))}
