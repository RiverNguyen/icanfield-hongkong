...
const FormInternationalJourney = ({
	...
	const formSchema = z.object({
		username: z
			.string()
			.min(2, { message: t('ten_nguoi_dung_phai_co_it_nhat_2_ky_tu') }) // Tùy chỉnh message cho min
													...
													!field.value && (
														<div className='text-placeholder hidden items-center'>
															<p className='text-start text-greyscaletext-800 body16-m'>
																{t('chuong_trinh_dinh_cu_ban_quan_tam')}
															</p>
											...
											{Array.isArray(dataNationSettlement) &&
												dataNationSettlement?.map((e: Term, index: number) => (
													<SelectItem
														...
														value={t('chuong_trinh_dinh_cu') + e?.slug}
													>
														{t('chuong_trinh_dinh_cu')} {e?.name || ''}
													</SelectItem>
												))}
										</SelectContent>
									</Select>
