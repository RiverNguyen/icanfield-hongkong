...
const ItemProjectsOutstanding = (data: Project) => {
  return (
    <Link
      href={data?.link || ''}
      className='overflow-hidden rounded-[1.25rem] border-[0.8px] border-[rgba(0,0,0,0.08)] bg-white shadow-[0px_4px_11.1px_0px_rgba(114,114,114,0.08)] group/item'
    >
      <div className='relative h-[18.5625rem] w-full overflow-hidden xsm:h-[14.25rem]'>
        <ImageV2
          ...
          width={500}
          height={500}
          className='h-full w-full object-cover group-hover/item:scale-[1.1] transition-all duration-300'
        />
        <div className='absolute bottom-0 left-0 right-0 z-10 bg-[linear-gradient(180deg,rgba(92,50,30,0.00)0.15%,rgba(40,14,2,0.90)95.57%)] w-full h-full'></div>
      ...
      <div className='px-[1.5rem] py-[1.25rem]'>
        <div className='grid grid-cols-2 col gap-x-[6rem] xsm:grid-cols-[3fr_2fr] xsm:gap-x-0 gap-y-[1.12rem] xsm:gap-y-3 [&>div]:flex-col [&>div]:space-y-1 [&>div_p]:line-clamp-1 [&>div_p]:text-[1rem] [&>div_p]:font-bold [&>div_p]:leading-[1.33] [&>div_p]:tracking-[-0.02rem] [&>div_p]:text-orangetext-500 [&>div_span]:text-[0.875rem] [&>div_span]:leading-[1.5] [&>div_span]:tracking-[-0.00875rem] [&>div_span]:text-greyscaletext-200'>
          <div className=''>
            <span>Loại hình:</span>
            <p>{data?.type}</p>
          </div>
          ...
          <div>
            ...
            <p>{data?.ub5CapitalRatio}</p>
          </div>
          <div>
            <span>Việc làm tạo ra:</span>
            <p>{data?.jobsCreatedRatio}</p>
          </div>
        </div>
      </div>
      <div className='flex items-center justify-between border-t-[1px] border-[rgba(0,0,0,0.08)] px-[1.5rem] py-[1.25rem] pb-[1.5rem] xsm:justify-start xsm:space-x-2 xsm:p-4'>
        <div className='group flex items-center space-x-3'>
          ...
          <span className='text-[0.875rem] font-medium leading-[1.5] tracking-[-0.0175rem] text-Phase-1-Brown xsm:hidden'>
            Liên hệ tư vấn
          </span>
        </div>
