...
const Header = ({
	...
	const slugLanguage = getLocaleSlug(locale)
		...
		<header className='fixed left-0 top-0 z-[1000] w-full'>
									...
									data.news_business.outstanding_news.map(
										(item: OutstandingPost, index) => (
											...
											>
												<Link
													href={
														`${getLocaleSlug(locale)}/blogs/${item?.post_name}` ||
														'/'
													}
			...
			<div className='relative flex h-[3.75rem] items-center justify-between bg-white py-[0.62rem] section-container sm:hidden'>
				...
				<div className='flex items-center space-x-[1.5rem]'>
					...
					>
						<span
							className={`absolute left-1/2 block h-[0.13rem] -translate-x-1/2 transform rounded-full bg-[linear-gradient(97deg,#5C321E_-3.86%,_#95502F_51.97%,#F5C178_117.18%)] transition-all duration-300 ease-in-out ${isCloseMenu
								? 'top-1/2 w-[1.25rem] -translate-y-1/2 rotate-45'
								: 'le top-[0.75rem] w-[1.63rem] translate-y-0 rotate-0'
								}`}
						/>
						<span
							className={`absolute left-1/2 block h-[0.13rem] w-[1.25rem] transform rounded-full transition-all duration-300 ease-in-out ${isCloseMenu
